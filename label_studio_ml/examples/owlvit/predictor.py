...
USE_COMBINED_RESULT = os.environ.get("USE_COMBINED_RESULT", False)
...
class DINOBackend(LabelStudioMLBase):
    ...
    def __init__(self, project_id, **kwargs):
        ...
        self.use_combined = USE_COMBINED_RESULT
    ...
    def one_task(self, task):
        ...
        if self.use_combined:
            predictions.append(self.get_combined_results(img_path, all_points, all_scores, all_lengths))
        elif self.use_ms or self.use_sam:
    ...
    def multiple_tasks(self, tasks):
        ...
        if self.use_combined:
            sam_results = self.get_batched_sam_results(batched_output)

            predictions = []

            for boxes_xyxy, (H, W), logits, sam_result in zip(box_by_task, lengths, logits, sam_results):
                points = boxes_xyxy.cpu().numpy()
                ...
                dino_result = self.get_results(all_points, all_scores, all_lengths)
                predictions.append({
                    'result': dino_result['result'] + sam_result['result']
                })
    ...
    def get_combined_results(self, img_path, all_points, all_scores, all_lengths):
        results = self.get_results(all_points, all_scores, all_lengths)['result'] +\
                  self.get_sam_results(img_path, all_points, all_lengths)['result']
